USAGE: 
Run the project and a console window will show up. Type in either "funnel2" or "bonus1" (without quotes) to choose which mode to run it in. In "funnel2" mode, enter a big string to get the length of the largest word funnel that can be created from that word. In "bonus1" mode, the program will run through the entire word list and stop at the first (and supposedly only) word in the list that creates a word funnel of length 10. Note that this runs for a while, and to spoil it - the answer is "complecting".
...
Thanks to u/duetosymmetry
for posting today's challenge on r/dailyprogrammer_ideas!